CREATE DATABASE project_sql_102;
...
-- create table with avg >= 90 
CREATE table exlent_students
SELECT * FROM students 
WHERE avrage >= 90;

SELECT * FROM exlent_students
ORDER BY avrage DESC;
DROP TABLE exlent_students;

-- create table to faild student
CREATE TABLE faild_students
SELECT * FROM students
WHERE avrage < 60;

SELECT * FROM faild_students;

-- show students starts with a 

SELECT * FROM students 
WHERE name like 'a%';

-- show students with for letters 
SELECT * FROM students 
WHERE name like '----';

-- use aggregate function with avrage and add name to result 
SELECT AVG(avrage) AS 'avrage' FROM students;
SELECT MAX(avrage) AS 'max' FROM students;
SELECT MIN(avrage) AS 'min' FROM students;
SELECT sum(avrage) AS 'sum' FROM students;
SELECT count(avrage) AS 'count' FROM students;

-- SHOW EXLENT STUDENT IN LEVEL 1
SELECT * FROM students 
WHERE level = 1 AND avrage >= 90;

-- find age 
SELECT TIMESTAMPDIFF(YEAR,birthdate,CURDATE()) as 'age' from students
 
 -- level 2 
select * from students
where level = 2;
-- show path without deplucate 
SELECT DISTINCT(path) from students;

-- show path in upper case 
SELECT UPPER(path ) FROM students ;

-- cheange  gender from f > female and m > male
UPDATE students 
...
-- add 5 mareks to students less than 70
